 ...
 *  both compile-time (1) and run-time initialized Lookup tables for efficient
 * CRC8 calculation. You can store multiple tables for different polynomials and
 * (non-)reflected CRCs. The different tables are referenced by an index, with
 * an upper limit set at compile time (CRC_TABLE_COUNT).
 *
 *  To generate CRCs you must first generate the Lookup-table by calling
 * fillCRCTable() with any index. CRCs can then be generated from any data
 * buffer by calling CRC() with the same index previously given to
 * fillCRCTable().
 *
 ...
 *  this algorithm could probably be speed up by preparing a 2- or 4-bit lookup
 * table to speed up the actual table generation.
 *
 *  (1): For compile-time CRC tables, just fill the table(s) by initializing
 * CRCTables[] to the proper values. (2): Tested by toggling a GPIO pin,
 * generating a table in-between and measuring the GPIO pulse width.
 */
...
uint8_t tmc_fillCRC8Table(uint8_t polynomial, bool isReflected, uint8_t index)
{
    ...
    if (index >= CRC_TABLE_COUNT) return 0;
    ...
    // Iterate over all 256 possible uint8_t values, compressed into a uint32_t
...
uint8_t tmc_CRC8(uint8_t *data, uint32_t bytes, uint8_t index)
{
    ...
    while (bytes--) result = table[result ^ *data++];
...
{
    if (index >= CRC_TABLE_COUNT) return false;
